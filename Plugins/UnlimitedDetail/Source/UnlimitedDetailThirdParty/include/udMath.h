...
//! @file udMath.h
//! Provides math types and functions for udSDK, including vectors, quaternions, matrices, and common math utilities.
//! All types are templated on element type T, with common typedefs provided for float and double variants.
...
#define UD_PI            3.1415926535897932384626433832795 //!< Pi (double precision)
#define UD_2PI           6.283185307179586476925286766559  //!< 2*Pi (double precision)
#define UD_HALF_PI       1.5707963267948966192313216916398 //!< Pi/2 (double precision)
#define UD_ROOT_2        1.4142135623730950488016887242097 //!< Square root of 2 (double precision)
#define UD_INV_ROOT_2    0.70710678118654752440084436210485 //!< 1/sqrt(2) (double precision)
#define UD_RAD2DEGVAL    57.295779513082320876798154814105 //!< Radians to degrees conversion factor (double precision)
#define UD_DEG2RADVAL    0.01745329251994329576923690768489 //!< Degrees to radians conversion factor (double precision)
#define UD_RAD2DEG(rad)  ((rad)*UD_RAD2DEGVAL) //!< Convert radians to degrees (double precision)
#define UD_DEG2RAD(deg)  ((deg)*UD_DEG2RADVAL) //!< Convert degrees to radians (double precision)


#define UD_PIf           float(UD_PI)           //!< Pi (single precision)
#define UD_2PIf          float(UD_2PI)          //!< 2*Pi (single precision)
#define UD_HALF_PIf      float(UD_HALF_PI)      //!< Pi/2 (single precision)
#define UD_ROOT_2f       float(UD_ROOT_2)       //!< Square root of 2 (single precision)
#define UD_INV_ROOT_2f   float(UD_INV_ROOT_2)   //!< 1/sqrt(2) (single precision)
#define UD_RAD2DEGVALf   float(UD_RAD2DEGVAL)   //!< Radians to degrees conversion factor (single precision)
#define UD_DEG2RADVALf   float(UD_DEG2RADVAL)   //!< Degrees to radians conversion factor (single precision)
#define UD_RAD2DEGf(rad) ((rad)*UD_RAD2DEGVALf) //!< Convert radians to degrees (single precision)
#define UD_DEG2RADf(deg) ((deg)*UD_DEG2RADVALf) //!< Convert degrees to radians (single precision)

#define UD_EPSILON       float(1.0f / 1024.0f) //!< Default epsilon for approximate comparisons
...
//! @defgroup udMath Functions
//! Wrappers around standard math library functions. Float and double overloads are provided for each.
//! @{

float udPow(float f, float n);         //!< Raise @p f to the power @p n
double udPow(double d, double n);      //!< Raise @p d to the power @p n
float udExp(float f);                  //!< e raised to the power @p f
double udExp(double d);                //!< e raised to the power @p d
float udLogN(float f);                 //!< Natural logarithm of @p f
double udLogN(double d);               //!< Natural logarithm of @p d
float udLog2(float f);                 //!< Base-2 logarithm of @p f
double udLog2(double d);               //!< Base-2 logarithm of @p d
float udLog10(float f);                //!< Base-10 logarithm of @p f
double udLog10(double d);              //!< Base-10 logarithm of @p d
float udRSqrt(float f);                //!< Reciprocal square root of @p f (1/sqrt(f))
double udRSqrt(double d);              //!< Reciprocal square root of @p d (1/sqrt(d))
float udSqrt(float f);                 //!< Square root of @p f
double udSqrt(double d);               //!< Square root of @p d
float udSin(float f);                  //!< Sine of @p f (radians)
double udSin(double d);                //!< Sine of @p d (radians)
float udCos(float f);                  //!< Cosine of @p f (radians)
double udCos(double d);                //!< Cosine of @p d (radians)
float udTan(float f);                  //!< Tangent of @p f (radians)
double udTan(double d);                //!< Tangent of @p d (radians)
float udSinh(float f);                 //!< Hyperbolic sine of @p f
double udSinh(double d);               //!< Hyperbolic sine of @p d
float udCosh(float f);                 //!< Hyperbolic cosine of @p f
double udCosh(double d);               //!< Hyperbolic cosine of @p d
float udTanh(float f);                 //!< Hyperbolic tangent of @p f
double udTanh(double d);               //!< Hyperbolic tangent of @p d
float udASin(float f);                 //!< Arc sine of @p f, result in radians
double udASin(double d);               //!< Arc sine of @p d, result in radians
float udACos(float f);                 //!< Arc cosine of @p f, result in radians
double udACos(double d);               //!< Arc cosine of @p d, result in radians
float udATan(float f);                 //!< Arc tangent of @p f, result in radians
double udATan(double d);               //!< Arc tangent of @p d, result in radians
float udATan2(float y, float x);       //!< Arc tangent of @p y / @p x using signs to determine quadrant, result in radians
double udATan2(double y, double x);    //!< Arc tangent of @p y / @p x using signs to determine quadrant, result in radians
float udASinh(float f);                //!< Inverse hyperbolic sine of @p f
double udASinh(double d);              //!< Inverse hyperbolic sine of @p d
float udACosh(float f);                //!< Inverse hyperbolic cosine of @p f
double udACosh(double d);              //!< Inverse hyperbolic cosine of @p d
float udATanh(float f);                //!< Inverse hyperbolic tangent of @p f
double udATanh(double d);              //!< Inverse hyperbolic tangent of @p d

//! @}

//!
//! Normalise a rotation angle into the range [-absRange, absRange].
//! @param rad The angle in radians to normalise.
//! @param absRange The half-range; the result will be in [-absRange, absRange]. Defaults to PI.
//! @return The normalised angle.
//!
template <typename T>
T udNormaliseRotation(T rad, T absRange = UD_PI);

//! Returns non-zero if @p x is a finite value (not infinity or NaN). For consistency with udArrayIsFinite.
#define udIsFinite(x) isfinite(x)

//!
//! Check whether all elements of an array are finite (not infinity or NaN).
//! @param pArr Pointer to the array of values.
//! @param n Number of elements to check.
//! @return True if all @p n elements are finite.
//!
...
//! @defgroup udMath Min/Max
//! Scalar and vector min/max functions.
//! @{
template <typename T> T             udMax(T a, T b) { return (a > b) ? a : b; } //!< Return the larger of @p a and @p b
template <typename T> T             udMin(T a, T b) { return (a < b) ? a : b; } //!< Return the smaller of @p a and @p b
template <typename T, typename U> T udMax(T a, U b) { return (a > (T)b) ? a : (T)b; } //!< Return the larger of @p a and @p b (mixed types, result is type T)
template <typename T, typename U> T udMin(T a, U b) { return (a < (T)b) ? a : (T)b; } //!< Return the smaller of @p a and @p b (mixed types, result is type T)
//! @}

//! @defgroup udMathRounding Rounding Functions
//! Round, floor, and ceil operations for scalars and vectors.
//! @{
...
//! @defgroup udMathLinearAlgebra Linear Algebra Functions
//! Vector operations including abs, min, max, clamp, dot, cross, normalize, magnitude, and interpolation.
//! @{
...
//! @defgroup udMathMatrixQuat Matrix and Quaternion Functions
//! Multiplication, transposition, inverse, determinant, lerp, slerp, and direction conversion utilities.
//! @{
...
template <typename T> udVector3<T> udDirectionFromYPR(const udVector3<T> &ypr); //!< Compute a unit direction vector from yaw/pitch/roll angles (radians)
template <typename T> udVector3<T> udDirectionToYPR(const udVector3<T> &direction); //!< Compute yaw/pitch/roll angles (radians) from a unit direction vector (roll will be 0)
//! @}
...
//!
//! @struct udVector2
//! A 2-component vector templated on element type T.
//! Provides component-wise arithmetic operators and common static factory methods.
//!
template <typename T>
struct udVector2
{
...
//!
//! @struct udVector3
//! A 3-component vector templated on element type T.
//! Provides component-wise arithmetic operators and common static factory methods.
//!
template <typename T>
struct udVector3
...
//!
//! @struct udVector4
//! A 4-component vector templated on element type T.
//! Provides component-wise arithmetic operators and common static factory methods.
//!
template <typename T>
struct udVector4
...
//!
//! @struct udQuaternion
//! A unit quaternion templated on element type T, used to represent 3D rotations.
//! Stored as (x, y, z, w) where w is the scalar component.
//! The quaternion is assumed to be normalized for rotation operations.
//!
template <typename T>
struct udQuaternion
{
  T x; //!< X imaginary component
  T y; //!< Y imaginary component
  T z; //!< Z imaginary component
  T w; //!< W scalar component
  typedef T ElementType;
  enum { ElementCount = 4 };

  const udVector4<T>& toVector4() const { return *(udVector4<T>*)this; }

  bool operator ==(const udQuaternion<T> &q) const { return x == q.x && y == q.y && z == q.z && w == q.w; }
  bool operator !=(const udQuaternion<T> &q) const { return !(*this == q); }
  ...
  udQuaternion<T>& inverse();   //!< Invert this quaternion in-place and return a reference to it
  udQuaternion<T>& conjugate(); //!< Conjugate this quaternion in-place (negate x,y,z) and return a reference to it

  udVector3<T> apply(const udVector3<T> &v) const; //!< Rotate vector @p v by this quaternion and return the result

  udVector3<T> eulerAngles() const; //!< Extract yaw/pitch/roll Euler angles (radians) from this quaternion
...
//!
//! @struct udRay
//! Represents a ray in 3D space with an origin position and a direction vector.
//!
template <typename T>
struct udRay
{
  udVector3<T> position;  //!< The origin point of the ray
  udVector3<T> direction; //!< The direction of the ray (should be normalized)
...
//!
//! @struct udPlane
//! Represents an infinite plane in 3D space, defined by a point on the plane and a surface normal.
//!
template <typename T>
struct udPlane
{
  udVector3<T> point;  //!< A point on the plane
  udVector3<T> normal; //!< The surface normal of the plane (should be normalized)

  //!
  //! Test whether a ray intersects this plane.
  //! @param ray The ray to test against the plane.
  //! @param pIntersectionPoint If not NULL, receives the world-space intersection point on success.
  //! @param pIntersectionDistance If not NULL, receives the distance along the ray to the intersection point on success.
  //! @return True if the ray intersects the plane in the forward direction.
  //!
...
//!
//! @struct udMatrix4x4
//! A column-major 4x4 matrix templated on element type T.
//! Columns are stored contiguously in memory; access via the `axis` member gives named column vectors,
//! while the `m` member provides individually indexed elements (note: element naming appears transposed
...
struct udMatrix4x4
{
  ...
  union
  {
    T a[16]; //!< Raw array access, column-major order
    udVector4<T> c[4]; //!< Column vector access
    struct
    {
      udVector4<T> x; //!< X axis column
      udVector4<T> y; //!< Y axis column
      udVector4<T> z; //!< Z axis column
      udVector4<T> t; //!< Translation column
    } axis;
    struct
    {
  ...
  udMatrix4x4<T>& transpose();   //!< Transpose this matrix in-place and return a reference to it
  T determinant();               //!< Compute and return the determinant of this matrix
  udMatrix4x4<T>& inverse();     //!< Invert this matrix in-place and return a reference to it

  udVector3<T> extractYPR() const; //!< Extract yaw/pitch/roll Euler angles (radians) from the rotation part of this matrix
  udQuaternion<T> extractQuaternion() const; //!< Extract the rotation of this matrix as a normalized quaternion
  //!
  //! Decompose this matrix into its constituent transforms.
  //! @param position Receives the translation component.
  //! @param scale Receives the per-axis scale factors.
  //! @param orientation Receives the rotation as a normalized quaternion.
  //!
...
//! @defgroup udMathTypedefs Convenience Typedefs
//! Typed aliases for common vector and matrix instantiations.
//! @{
